 ...
 */const badgeCls =
  "inline-block rounded-full bg-green-100 text-green-800 px-2 py-0.5 text-xs font-medium";const btnPrimaryCls =
  "flex items-center justify-center gap-1 rounded-md bg-orange-500 hover:bg-orange-600 text-white text-sm h-9 px-3 w-full";const btnOutlineCls =
  "flex items-center justify-center rounded-md border border-gray-300 hover:bg-gray-50 text-gray-600 h-9 w-9";function IndividualRecipePage() {
  const { state } = useLocation();
  const recipe = state?.recipe;  if (!recipe) {
    return (
